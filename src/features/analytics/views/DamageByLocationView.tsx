...
const STAT_METRICS = [
  ...
  { key: 'locationsCount', title: 'Affected Locations', subtitle: 'Locations with damage', unit: '' },
...
export function DamageByLocationView({
  ...
  const primaryRanking = useMemo(
    () => data.ranking.map(r => ({ location: r.location, yield: r.total_affected_area })),
    [data.ranking],
  );
  ...
  const transformedCmpData = useMemo(() =>
    hasComparison
      ? cmpDataItems.map((items: any[]) =>
        Array.isArray(items)
          ? items.map((item: any) => ({ location: item.location, yield: item.compare ?? 0 }))
          : [],
      )
      : [],
  ...
  const seasonColorMap = useMemo<Record<string, string>>(() => {
    ...
    list.push(...cmpLabels);
    const shades = generateShades(DAMAGE_BASE, list.length);
  ...
  const trends = useTrendData(
    ...
    currentLabel,
    isAllSeasons,
    ['totalReports', 'totalArea', 'locationsCount', 'avgArea'],
  );
    ...
    <div className="flex flex-col gap-4">
      ...
      {isAllSeasons && (
        <div className="grid auto-rows-min gap-4 md:grid-cols-4">
          ...
          />
          <StatCardSparkline
            title="Overall Affected Locations"
            subtitle="Locations with damage"
            value={locationsWithData}
            unit=""
            trend={trends.locationsCount}
            inverted
          />
          <StatCardSparkline
            title="Overall Avg Area per Location"
      ...
      {!isAllSeasons && !hasComparison && (
        <div className="grid auto-rows-min gap-4 md:grid-cols-4">
          ...
          <StatCardMinimal title="Affected Locations" subtitle="Locations with damage" current_value={locationsWithData} unit="" />
      ...
      {locationKeys.length > 0 && (
        <MultiLineChart
          key={`damage-location-${locationKeys.length}`}
          data={lineRows}
          categoryKey="season"
...
function buildInsight(
  ...
  if (data.length === 1) {
    return `${data[0].name} recorded ${data[0].value.toFixed(metric === 'locationsCount' ? 0 : 2)} ${unit}.`;
  }
  ...
  switch (metric) {
    ...
    case 'locationsCount':
      return `${max.name} had damage in ${max.value} locations. ${min.name} was affected in ${min.value} locations.`;
    case 'avgArea':
      return `${max.name} has the highest average area per location (${max.value.toFixed(2)} ${unit}); ${min.name} has the lowest (${min.value.toFixed(2)} ${unit}).`;
    default:
      return '';
  }
}
