...
begin
    if p_season_id is null then
        -- Aggregate across every season, no previous comparison
        has_previous := false;
        ...
        select
            ...
            coalesce(count(*) filter (where da.observed_pest is not null
                                      and da.observed_pest != ''
                                      and da.observed_pest != 'N/A'), 0)
        ...
        select coalesce(round(avg((hr.bags_harvested * hr.avg_bag_weight_kg) / 1000.0 / hr.area_harvested_ha)::numeric, 2), 0)
        into current_yield
        from public.field_activities fa
        join public.harvest_records hr on hr.id = fa.id
        where hr.area_harvested_ha > 0
        ...
        into current_total_area, current_not_sufficient
        from public.field_activities fa
        join public.harvest_records hr on hr.id = fa.id
        where hr.area_harvested_ha > 0
          and (hr.bags_harvested * hr.avg_bag_weight_kg) > 0;
        ...
        -- current_yield := case
        --     when current_total_area > 0 then round((current_total_yield / current_total_area) / 1000, 2)
        --     else 0
        -- end;
