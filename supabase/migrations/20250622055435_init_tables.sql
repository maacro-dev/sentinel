...
/*
  This table holds general metadata common to all activity types.

  Each record in this table [must be associated] with exactly one record
  in one of the following child tables, which store the activity-specific details:
    - field_plannings
    - crop_establishments
  ...
  Per season, the collection should be:
    1 field, 6 field_activities (each of the child tables)
      - field_planning
      - crop_establishment
      - fertilization_record
      - crop_cut
...
create table field_plannings ( -- field_data / metadata 1
  ...
  ecosystem                         text not null, -- irrigation, rainfed
  soil_type                         text not null, -- clayey, loamy, etc.
  current_field_condition           text not null, -- planted, harvested, land prep, etc.
  crop_planted                      text not null, -- rice
  crop_status                       text not null -- tillering, etc.
);
...
create table crop_establishments ( -- cultural_mgmt / metadata 2
  id                                int primary key references field_activities(id) unique,
  ...
  actual_land_preparation_method    text not null, -- wet, dry
  actual_crop_establishment_date    date not null,
  actual_crop_establishment_method  text not null, -- transplanted, direct-seeded

  -- if transplanted
  sowing_date                       date null, 
  seedling_age_at_transplanting     smallint null check (seedling_age_at_transplanting between 1 and 100),
  distance_between_plant_row_1      double precision null, -- 10cm-15cm
  distance_between_plant_row_2      double precision null, -- 10cm-15cm
  distance_between_plant_row_3      double precision null, -- 10cm-15cm
  distance_within_plant_row_1       double precision null, -- 10cm-15cm
  distance_within_plant_row_2       double precision null, -- 10cm-15cm
  distance_within_plant_row_3       double precision null, -- 10cm-15cm

  -- if direct-seeded
  seeding_rate_kg_ha                double precision null,
  direct_seeding_method             text null, -- broadcasted, row direct-seeded
...
create table fertilization_records ( -- nutrient_mgmt / metadata 3
  id                                int primary key references field_activities(id) unique,
  applied_area_sqm                  double precision not null
);

/*
  Child table of fertilization_records
  Could be taken more than once
*/
create table fertilizer_applications ( -- included in nutrient_mgmt
  id                                int generated by default as identity primary key,
  ...
  -- in %
  nitrogen_content                  double precision not null,
  phosphorus_content                double precision not null,
  potassium_content                 double precision not null,
...
create table harvest_records ( -- production / metadata 4
  ...
  harvesting_method                 text not null, -- manual, mechanical
  ...
  irrigation_supply                 text not null -- not enough, sufficient, excessive
);
